...
| Above the deletion floor, fixed-geometry contraction has an explicit compute-to-target bound `C_S K_S(R_tar)` | theorem | `theory/parts/07_compute_to_accuracy.tex` |
...
| If the master risk bound is below the risk-equivalent measured compute saving, contraction dominates in `risk + tau * compute` | conditional theorem | compute-aware corollary to master theorem |
...
| A finite independent probe gives a simultaneous paired empirical-Bernstein UCB on structural damage over a finite candidate family | theorem | `theory/parts/06_finite_sample_observability.tex` |
...
| Target-value state predicts future probe structural damage better than time/size on development trajectories | empirical diagnostic | LO-trajectory-out `R^2=0.362`, Pearson `0.618` on 32 development transitions |
| Small probe structural damage is a reliable proxy for test structural damage in the current development setup | falsified / not supported | realized probe-vs-test damage Pearson `-0.581` |
| Target-value SRA improves the onset decision | open / development evidence only | small paired pilot is mixed and still compute-negative |
...
- We do **not** call a high probe-forecast `R^2` evidence for safe contraction when the structural damage itself is not statistically resolved.
